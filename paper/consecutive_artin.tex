...
($\chi^2/\mathrm{df} = 1.14$ on $633$ cells). The mechanism is the bias of
...
The per-prime dataset ($1.8$~GB CSV covering all primes to $10^9$), the C
sieve, and all analysis scripts and logs reproducing every number in this
paper are available from the author on request; the dataset is also exactly
reproducible from the (short) C source, which will be deposited with the
analysis scripts in a public repository upon acceptance.

\section*{Generative AI disclosure}
A large-language-model assistant (Claude, Anthropic, via the OpenClaw
platform) was used to draft the Python analysis scripts, to assist with the
literature search, and to draft and edit the prose of this manuscript,
including the initial formulation of the proof of Theorem~\ref{thm:exclusion}.
All computations were run and verified on the author's hardware; the proof has
been checked by hand and verified computationally; and the author has reviewed
all AI-assisted text and takes full responsibility for the correctness and
integrity of the work. No AI tool is listed as an author.
