...
\begin{enumerate}
	...
	\item (Section~\ref{sec:global}) Across all $50{,}847{,}530$ consecutive
	      prime pairs with $7 \le p_n \le 10^9$, Artin statuses are anticorrelated:
...
small nonzero limit. A Hardy--Littlewood-based channel model (not developed
here) would predict a nonzero limiting~$\delta$ from the non-uniform weights
of gap classes in the singular series, but such a prediction is conditional on
unproved conjectures and has not been verified against data at larger scales.
...
configurable via a command-line argument; all scripts accept their
input/output paths as arguments and default to repository-relative locations.
