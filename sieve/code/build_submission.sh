#!/bin/bash
# Rebuild the submission set from the CURRENT paper/consecutive_artin.tex.
# Produces, in submission/:
#   consecutive_artin_manuscript.pdf   named manuscript
#   consecutive_artin_anonymous.pdf    anonymized (author/thanks/ORCID/email stripped, metadata cleared)
#   consecutive_artin_source.zip       flat named LaTeX source + figure
#   consecutive_artin_reproduction.zip portable package (excludes archives + correspondence)
# Usage:  bash code/build_submission.sh
set -euo pipefail
cd "$(dirname "$0")/.."
ROOT="$PWD"
SUB="$ROOT/submission"
TEX="$ROOT/paper/consecutive_artin.tex"
WORK="$(mktemp -d)"
trap 'rm -rf "$WORK"' EXIT

echo "== named manuscript"
cd "$ROOT/paper"
for i in 1 2 3; do pdflatex -interaction=nonstopmode consecutive_artin.tex >/dev/null 2>&1; done
BOX=$(grep -cE '^(Overfull|Underfull)' consecutive_artin.log || true)
UND=$(grep -ciE 'undefined|multiply.defined' consecutive_artin.log || true)
PAGES=$(pdfinfo consecutive_artin.pdf | awk '/^Pages/{print $2}')
...
cp consecutive_artin.pdf "$SUB/consecutive_artin_manuscript.pdf"

echo "== anonymous manuscript"
cp "$TEX" "$WORK/anon.tex"
cp "$ROOT/paper/fig_gap_delta.pdf" "$WORK/"
...
cp anon.pdf "$SUB/consecutive_artin_anonymous.pdf"
...
rm -f "$SUB/consecutive_artin_source.zip"
cd "$ROOT/paper"
zip -q -X "$SUB/consecutive_artin_source.zip" consecutive_artin.tex fig_gap_delta.pdf
...
cp "$ROOT"/paper/consecutive_artin.{tex,pdf} "$ROOT"/paper/fig_gap_delta.{pdf,png} "$ROOT"/paper/make_figure.py "$REPRO/paper/"
cp "$ROOT"/results/* "$REPRO/results/" 2>/dev/null || true
cp "$SUB/consecutive_artin_anonymous.pdf" "$SUB/consecutive_artin_source.zip" "$SUB/SUBMISSION_CHECKLIST.md" "$REPRO/submission/"
...
rm -f "$SUB/consecutive_artin_reproduction.zip"
zip -qr -X "$SUB/consecutive_artin_reproduction.zip" .
if unzip -l "$SUB/consecutive_artin_reproduction.zip" | grep -qiE 'endorsement|archive/'; then
