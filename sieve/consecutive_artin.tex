...
\begin{abstract}
	...
	Across four decades the deficit strengthens monotonically with decelerating
	increments --- $\delta = -0.01336, -0.01414, -0.01472, -0.01500$ at
	$x = 10^8, 10^9, 10^{10}, 10^{11}$ --- which over the measured range does not
	support $\delta(x) \to 0$; we do not claim a nonzero limit, since the
	increment law is not identified.
...
We have measured $\delta$ over four decades:
\[
...
primes respectively; at $10^{11}$ the sampling standard error on $\delta$ is
about $1.6 \times 10^{-5}$). The correlation strengthens monotonically, and the
increments $-0.000780$, $-0.000583$, $-0.000275$ decelerate. Over the measured
range this does not support $\delta(x) \to 0$.

It does not, however, establish a nonzero limit, and we claim none. The
increment law is not identified: the ratio of successive increments is not
constant ($0.75$ then $0.47$), so a geometric model fails --- extrapolating
geometrically from the first three values predicts
$\delta(10^{11}) \approx -0.01516$, which the measurement misses by some ten
standard errors --- and $\delta(x)\log x$ is not constant either
($-0.246, -0.293, -0.339, -0.380$), so simple $1/\log x$ decay fits no better.
What the data support is the conjecture as stated, that $\delta$ remains
negative, together with the weaker observation that any decay to zero would
have to be slower than these four decades suggest. We note one
