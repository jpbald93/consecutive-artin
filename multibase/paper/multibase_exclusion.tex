...
proportion lies between $26.3\%$ and $27.8\%$ throughout, as one expects from the square of
...
$x = 10^9$, with the least-squares curve $C/\sqrt{f}$, $C = 0.132$. Right:
$|\delta(a)|$ against the weighted fraction of consecutive pairs lying in
exclusion classes. Bases admitting no exclusion law are shown in red; one of
them has the largest $|\delta|$ of all.}
\label{fig:conductor}
\end{figure}
...
Three directions suggest themselves. First, a derivation of
Observation~\ref{obs:conductor} from the singular series
of~\cite{HardyLittlewood1923} together with Hooley's density
computation~\cite{Hooley1967}, summed over the $\varphi(f)$ residue channels,
would replace an empirical regularity with a formula and settle the exponent
question. Second, the decay of $\delta(a)$ in $x$ should be measured for several
