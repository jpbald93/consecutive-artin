...
$p$. Membership in $\Art_a$ forces the quadratic character value
...
incidences below $10^9$. The two central theorems are additionally
...
The exclusion weight $w$ correlates with $|\delta|$ at $r = 0.646$, but this
...
positive value for all eight bases that possess such classes. The data
indicate that the conductor of $\chi_a$, not the exclusion structure, is the
parameter organising the global correlation.
\end{abstract}
...
constant. It is known under GRH by Hooley~\cite{Hooley1967}. Unconditionally,
Gupta--Murty~\cite{GuptaMurty1984} and Heath-Brown~\cite{HeathBrown1986}
established that Artin's conjecture holds for all but at most finitely many
primes as bases, and that at most two primes can be exceptional; in
particular, of any three distinct primes, at least one is a primitive root for
infinitely many $p$. See Moree's survey~\cite{Moree2012} for the extensive
...
bounded-gap pairs of primes sharing that primitive root, and
Baker--Pollack~\cite{BakerPollack2016} proved an unconditional result of this
type for primitive roots drawn from a large set of primes.
...
\begin{itemize}
\item when a gap class \emph{is} a quadratic exclusion class, no pair of primes
      at that gap can both be Artin base $a$ --- an unconditional theorem;
...
there. Pollack~\cite{Pollack2014} and Baker--Pollack~\cite{BakerPollack2016}
produce bounded-gap prime pairs sharing a primitive root, but impose no
structure on the gap classes and do not identify forbidden ones. We likewise
searched for forbidden-gap statements for quadratic characters in other
contexts --- consecutive quadratic residues and non-residues have a classical
literature --- and found constraints on character values along progressions,
but no statement coupling a gap \emph{class} to the joint primitive-root
status of a pair of primes, which is the content here. In that sense
``apparently unrecorded'' above is meant as the outcome of a search, not as an
impression.

Our main theorem determines, for every base, exactly which gap classes are
quadratic exclusion classes.
...
The hypothesis $g \not\equiv 0 \pmod d$ is not cosmetic. At $g \equiv 0$ the
displayed formula returns $(d-3+2)/4 = (d-1)/4$, which is not in general an
integer --- for $d = 13$ it gives $2.5$ against a true count of $6$ --- so the
$g \equiv 0$ case must be handled separately, as it is above. Both statements
are machine-checked (Section~\ref{sec:formal}).
...
modulo $12$ are exclusion classes, and for $d = 5$ two of the four
...
\item the three bases with no exclusion classes at all ($a = 13, 17, 29$)
      still exhibit $|\delta|$ from $0.022$ to $0.043$ --- larger than seven of
...
\item restricting $\delta$ to gap classes outside the exclusion set
      ...
      bases possessing such classes: the global negative $\delta$ arises as a
      competition between forced-zero classes and positively coupled ones, not
...
as a measured empirical regularity (Observation~\ref{obs:conductor}) rather
than a conjectured exact power of $f$, because the data do not support a clean
exponent.
...
population is deterministic --- and it should be read as an indication that
the quoted digits of $\delta$ are not numerical noise, not as a test of any
hypothesis. Correlations across the eleven bases are descriptive summaries of
...
root. For instance $\chi_2(7) = -1$ while $2$ has order $3$ modulo $7$.
...
\begin{definition}
\label{def:exclusion}
A class $g \bmod f$ is a \emph{quadratic exclusion class} for base $a$ if there
is no residue $r \in R_f$ with $r + g \in R_f$ and
...
exclusion class. That gap is closed in Section~\ref{sec:identity}.
...
even. If $f$ is even, replacing $g$ by $g + f$ if necessary preserves the class
modulo $f$ and adjusts the parity, so an even representative exists. If $f$ is
odd then $D_j = -3$ and $f = |D| = d$ is odd; here the reversing condition is
...
would give $(d-1)/4$, which for $d = 13$ is $2.5$ --- not an integer, against a
true count of $6$. The error is invisible at $d = 5$, where both expressions
happen to be small, which is presumably how it survived. We record it because
...
class modulo $f$ and testing Definition~\ref{def:shift} exhaustively over all
even $g$ modulo $f$. The resulting sets agree \emph{exactly}, in every case,
...
shift modulo $f$, a total of $15{,}639$ (base, shift) pairs, with no
discrepancy. The criterion of the complete dichotomy
(Corollary~\ref{cor:complete}) was confirmed by exhaustive scan --- over all
residue classes modulo $f$, odd classes included --- for all $72$ non-square
$a$ with $2 \le a \le 80$, again with no discrepancy.
Table~\ref{tab:classification} records a representative sample.

\section{Machine verification}
\label{sec:formal}

The two theorems on which the deterministic half of this paper rests have been
formalised and checked by the Lean~4 proof assistant against
\texttt{mathlib}. We report this because both statements were, in an earlier
...
appeal to \texttt{native\_decide}; every theorem depends only on Lean's three
standard axioms (\texttt{propext}, \texttt{Classical.choice},
\texttt{Quot.sound}), and a check script verifies this. We state plainly what
is \emph{not} formalised. The general-$d$ case of
Theorem~\ref{thm:inadmissible} is not, because it requires the Jacobi-type
evaluation $\sum_r \chi(r)\chi(r+g) = -1$, which is not at present available
in \texttt{mathlib}. The reversing classification of
...
$\ell \mid p-1$. Modular exponentiation was carried out in $128$-bit
arithmetic. Sharing the factorization of $p-1$ across all eleven bases is what
makes the computation inexpensive; the entire run takes under an hour on a
single core and writes no intermediate table, accumulating instead the
...
lies between $26.3\%$ and $29.8\%$. We do not offer a closed-form explanation
...
notice. A supplementary check reproduced the same zero counts, independently,
...
$r(w, |\delta|) = 0.646$ at $x = 10^9$ --- but the association does not
survive scrutiny:
\begin{enumerate}
...
\item restricting $\delta$ to the pairs \emph{outside} the exclusion classes
      ...
      the no-exclusion bases are untouched by construction. The global
      negative $\delta$ of the exclusion-law bases is thus an average of
      forced-zero classes against positively coupled ones, whereas for
      $a = 13, 17, 29$ it is a genuine within-class residue effect.
\end{enumerate}
...
coupled --- push it positive, in some cases substantially (base $3$ outside its
...
the \emph{composition} of $\delta$, not its size. Table~\ref{tab:outside} is a
restriction of the statistic to a subset of gap classes, not an additive
decomposition of the global $\delta$ into contributions.
...
conductor of $\chi_a$, and this single parameter orders the observed data
better than any exclusion-based quantity. The dependence is not perfectly
...
$|\delta| \cdot \sqrt{f}$ range over $0.0714$ to $0.1758$; the latter is the
...
($0.1758$), a factor of $2.5$ apart, so the fitted constant $C = 0.132$ in
...
\emph{deterministic quadratic exclusion laws} for consecutive Artin primes
...
(Corollary~\ref{cor:complete}), all resting on the counting identity of
Theorem~\ref{thm:identity}; and (iii) the first cross-base measurement of
...
$2$, so $g = 2$ is certainly not an exclusion class for base $3$. We know of
no base for which $g = 2$ is a quadratic exclusion class.
...
corrected here; the first two are now machine-checked. The author is solely
...
Tables~\ref{tab:verification} and~\ref{tab:delta}, scripts reproducing
Figure~\ref{fig:conductor}, and the Lean~4 development described in
Section~\ref{sec:formal} together with the script that checks its axiom
dependencies. The computation reported here can be reproduced from the
repository in under an hour on a single core; rebuilding the Lean development
