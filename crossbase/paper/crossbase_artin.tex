...
$|z|$ up to $476$ --- which reflects the $5 \times 10^7$ sample size rather
than a large effect; the correlations explain about $0.1\%$ of variance),
...
dependent pairs (mean absolute residual $0.018$ within dependent pairs, or
$0.008$ excluding one large positive outlier, versus $0.001$ for
multiplicatively independent pairs), and is localised in the quadratic-character
channel --- identifying it as the statistical residue of the exclusion law
itself, visible once the common cause is conditioned away. Finally we evaluate the joint
...
mean relative error of $0.012\%$, and they reproduce the sign reversal on the
entangled pairs. The deterministic exclusion law and the statistical residual
are thereby identified as two faces of one object: the quadratic character
$\chi_{\sqf(ab)}$, appearing pointwise as a congruence obstruction and
asymptotically as a degree collapse.
\end{abstract}
...
for which several prescribed integers are simultaneously primitive roots.
Lenstra's Galois-theoretic framework~\cite{Lenstra1977} covers a single
finitely generated group $W$ with a prescribed index condition, and Lenstra
explicitly lists the simultaneous problem --- ``the set of primes $q$ for which
both $2$ and $3$ are primitive roots'' (his~(8.17), citing Matthews) --- among
the sets to which his results do not directly apply. The systematic
Kummer-degree treatment of the simultaneous densities is due to
Moree--Stevenhagen~\cite{MoreeStevenhagen2014}, whose character-sum method
reproves Matthews' theorem; see also Moree's survey~\cite{Moree2012}. The qualitative message of
...
the fact that a quadratic residue is not a primitive root. The triple criterion
is moreover classical in the density theory, where the set of primes for which
$a_1, \ldots, a_n$ are simultaneously primitive roots is known to be empty
precisely when some odd-size subset $I$ has $\prod_{i \in I} a_i$ a square
(see~\cite[\S5, (5.2)]{MoreeStevenhagen2014}; the criterion is originally
Matthews'~\cite{Matthews1976}), of which Theorem~\ref{thm:triple} is the case
$n = 3$, $I = \{a,b,c\}$. What we add is the elementary pointwise proof, the
sharpening to odd $p$ with no further hypotheses, and --- the point of
emphasis here --- the observation that the third base is inessential: the
obstruction is a congruence, of density $1/2$, attached to the single character
$\chi_{\sqf(ab)}$. That character is the object the rest of the paper measures.
...
\begin{enumerate}
  ...
  \item \emph{Positivity, nonetheless.} $\phi(a,b) > 0$ for $64$ of $66$
    pairs, with mean $0.0352$ and $z$-scores up to $476$: knowing that $a$ is a
  ...
  \item \emph{Where the exclusion wins.} The only two negative
    correlations are $\phi(2,10) = -0.0302$ and $\phi(3,15) = -0.0303$. Both
    are multiplicatively dependent, but dependence alone is not the
    discriminant: fifteen of the $66$ pairs are dependent and thirteen of those
    are positively correlated. What singles out these two is the conductor of
    the barring character $\chi_{\sqf(ab)}$: both have $\sqf(ab) = 5$, of
    conductor $5$ --- the smallest conductor occurring among the dependent
    pairs, and the only odd prime one. A small conductor is what lets the
    exclusion of Theorem~\ref{thm:pair} survive the averaging over $p-1$
    instead of being diluted by it.
  ...
  \item \emph{What survives is entanglement, and it is exactly localised.}
    ...
    \emph{dependent} pairs (mean absolute residual $0.0179$, or $0.0077$
    excluding the single large positive outlier $(2,6)$, with individual values
    reaching $-0.034$ for $(2,10)$ and $-0.026$ for $(5,15)$) is eighteen times
    that of independent pairs ($0.0010$), or eightfold with $(2,6)$ excluded;
    and additionally conditioning on the quadratic-residue statuses
    $(\chi_a(p), \chi_b(p))$ flattens dependent and independent pairs to a
    common $+0.005$. The residual is thus identified, empirically, as the
    quadratic-character entanglement $\chi_a \chi_b = \chi_{\sqf(ab)}$ --- the
...
measurable second-order correction). What this paper adds to the pattern is
that the deterministic core and the statistical carrier are the \emph{same}
character seen twice: $\chi_{\sqf(ab)}$ bars half the primes pointwise, and
reappears as the degree collapse that bends the predicted densities.
...
\[
  ...
  (3,6) \text{ and } (5,10),\; d = 2\!: \ p \equiv 3, 5 \!\!\pmod 8; \qquad
...
simultaneously. Theorem~\ref{thm:pair} is also machine-checked in Lean~4
against Mathlib, as is the $(5,10)$ instance above with its barring condition
discharged from $p \bmod 8$; see \texttt{LEAN\_NOTE.md} in the accompanying
package.
...
In both theorems only ``$p$ odd'' is needed. Non-squareness of the bases is not
...
root mod $2$ and e.g.\ $(a,b,c) = (3,5,15)$ violates the conclusion there. The
accompanying Lean formalisation (Section~\ref{sec:data}) confirms
...
negligible, and the relevant quantity is the effect size. The $66$ pairs are
not independent tests --- twelve bases generate them and all share the same
primes --- but the smallest $\lvert z \rvert$ among them is $169$, against a
Bonferroni threshold of $3.4$ at the $5\%$ level, so no multiple-comparison
correction affects any conclusion below.
...
non-Artin. Pooling instead on the covariance scale over all cells, which
...
The sign reversal is Theorem~\ref{thm:pair} operating statistically. For every
pair, dependent or not, the primes split into two halves according to the sign
...
fifteen multiplicatively dependent pairs, $d = \sqf(ab)$ takes the values
...
cause away, after which the exclusion becomes visible on six dependent pairs
rather than two.
...
\section{The residual is the exclusion law}
\label{sec:entanglement}

The $5\%$ that survives is not noise, and it is not uniformly distributed:
it sits precisely on the multiplicatively dependent pairs. This section
identifies it as the statistical residue of Theorem~\ref{thm:pair} --- the
...
residuals, and every pair with a substantial \emph{negative} residual is
multiplicatively dependent. The six pairs below are the only ones with
$\phi \mid \mathrm{sig} < -0.004$, and all six are dependent (Fisher exact
$p = 5.5 \times 10^{-5}$):
\[
...
that sharper statement is not stable with $x$: at $x = 3 \times 10^7$ a further
twelve pairs have residuals marginally below zero, all within three standard
errors of it.

The residual is not sign-definite, and we record the exceptions explicitly.
Nine of the fifteen dependent pairs have \emph{positive} residuals: eight of
...
entanglement phenomenon seen from the other side: $\sqf(6) = 2 \cdot 3$, so
whenever $3 \nmid p-1$ the Artin conditions for $2$ and for $6$ very nearly
coincide, and the signature records whether $3 \mid p-1$ without recording the
...
than of a single one.) The dependent-group mean absolute residual of $0.0179$ is
substantially driven by this one pair: excluding it, the figure is $0.0077$,
still eightfold the independent-pair value of $0.0010$. Conditioning
...
character-level entanglement, and that it flattens the dependent pairs
identifies the residual as the statistical trace of the relation
$\chi_a \chi_b = \chi_{\sqf(ab)}$ --- the same object that makes
Theorem~\ref{thm:pair} true.

Indeed the identification can be made exactly, and not merely statistically.
...
\]
while on the complementary stratum the joint density is $0.299$. The
residual measured in this section is therefore not evidence \emph{for} a
conjectural mechanism: the mechanism is a theorem, and what the residual
measures is how much of that deterministic constraint survives the averaging
over $p-1$ that the unconditional correlation performs.
...
criterion~\eqref{eq:obstruction} an Artin prime for $a$ has $\chi_a(p) = -1$,
so all the Artin mass sits in the single quadrant $(-1,-1)$, and the remaining
three quadrants contribute no variation in either indicator. Conditioning on
...
\[
  ...
  \;+\;
  \underbrace{\text{quadratic entanglement } \chi_a\chi_b = \chi_{\sqf(ab)}}_{\text{residual, localised on dependent pairs}}
  \;+\;
...
$d_a d_b \sim \sqf(ab)$: the barring character $\chi_{\sqf(ab)}$ of
Theorem~\ref{thm:pair} appears in the model as an extra degree collapse,
...
$0.040\%$ (attained at $(21,29)$). In particular the model predicts the sign
...
Correspondingly, the deterministic law of Theorem~\ref{thm:pair} and the
statistical residual isolated in Section~\ref{sec:entanglement} are two faces
of one object: the character $\chi_{\sqf(ab)}$, which bars half the primes
pointwise and contributes the degree collapse in $V(m,n)$ asymptotically.
...
integer $p - 1$, with deterministic exclusion on an explicit half of all
primes for every pair of bases.
...
set so that several small odd prime conductors occur among the dependent
pairs, which our twelve bases do not provide. (3) The mixed axis: $\phi$
...
$\Q(\zeta_3)$ playing the role $\Q(i)$ and $\Q(\sqrt{2})$ played here. The
analogue of Theorem~\ref{thm:pair} at $\ell = 3$ would bar $2/3$ of primes on
a cubic-residue condition, and we do not know whether it is visible above the
common cause.
...
and debugging the sieve and analysis programs, for exploratory discussion, and
for an adversarial audit of an earlier version of this manuscript, which
located the $\varepsilon(m,n)$ error recorded in Remark~\ref{rem:eps} and the
omission of the $(2,6)$ residual. In addition, the skeleton of the Lean proof
of the primitive-root/non-residue bridge
(\texttt{PrimitiveRootBridge.lean}, criterion~\eqref{eq:obstruction}) was
drafted by a locally hosted open-weights model (Qwen3-32B) over six supervised
rounds and then completed by the author, who supplied the two steps the model
flagged as uncertain; provenance is recorded in the file's own header.

All computations were executed and verified on the author's own
hardware from source code the author has inspected. The proofs of
Theorems~\ref{thm:pair} and~\ref{thm:triple} have been checked by hand and
machine-checked in Lean~4 against Mathlib with no appeal to any axiom beyond
\texttt{propext}, \texttt{Classical.choice} and \texttt{Quot.sound}, including
the step \eqref{eq:obstruction} and, for the instance $(a,b) = (5,10)$, the
discharge of the barring condition $\bigl(\tfrac{2}{p}\bigr) = -1$ from
$p \bmod 8$ --- so that instance is verified with no character hypothesis
assumed at all. The general translation of $\bigl(\tfrac{d}{p}\bigr) = -1$ into
residue classes mod $4d$ is not formalised generically; it is carried out by
hand per instance. See \texttt{LEAN\_NOTE.md} in the accompanying package. Every reference has been
checked against the published record, with two stated exceptions: the
companion preprints~\cite{BaldI, BaldII} are unrefereed and have no published
record, and~\cite{Matthews1976} was verified from the publisher's
bibliographic record and from independent restatements of its main theorem
rather than from the original text, which the author was unable to obtain. The
...
derived (\texttt{crossbase\_qr\_1e9.json},
\texttt{crossbase\_fine\_1e9.json}, \texttt{crossbase\_om\_1e9.json}; $3.2$~MB
in total). Running \texttt{analyze\_qr.py} on the first of these reproduces
Table~\ref{tab:residual} exactly. The full computation reproduces in under two
...
unrefereed preprint (2026); code and data at
\url{https://github.com/jpbald93/consecutive-artin/tree/main/paper1}.

\bibitem{BaldII}
J.~Bald,
\emph{Exclusion laws for consecutive Artin primes in arbitrary bases},
unrefereed preprint (2026); code and data at
\url{https://github.com/jpbald93/consecutive-artin/tree/main/paper2}.
