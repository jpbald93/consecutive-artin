...
$\delta(15;x) \to 0$ as $x \to \infty$. More generally, for every base $a$,
$\delta(a;x) \to 0$, and the \emph{sign} of $\delta(a;x)$ for large $x$ is
that of $T(a)$ computed with the limiting gap-class weights, for every $a$
with $T(a)$ bounded away from $0$.
\end{conjecture}

The fast observed decay admits a natural suspect: the positive term in the
class competition is itself proportional to the \emph{same} LOS transition
bias that drives the within-class repulsion --- both decay together, and the
measured $\delta(15)$ is a small difference of two decaying quantities. A
difference of two terms each of order $1/\log x$ can decay like
$1/(\log x)^2$ or faster without either term doing so. Our two-point data
cannot distinguish $1/(\log x)^2$ from other super-$1/\log x$ rates; a
$10^{11}$--$10^{12}$ run for this single base would.
The companion paper on the conditional Hardy--Littlewood model
addresses the same question from the model side.
